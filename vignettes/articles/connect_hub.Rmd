...
All data returned from connecting to and querying hubs can be read or validated as a `model_out_tbl` which is an S3 class defined by the `hubUtils` package. A `model_out_tbl` is a long-form `data.frame` designed to conform to the [hubverse data specifications for model output data](https://hubdocs.readthedocs.io/en/latest/format/model-output.html). In short, the columns of a valid `model_out_tbl` containing model output data from a hub are:
 ...
 - `...` : other columns will be present depending on modeling tasks defined by the individual modeling hub. These columns are referred to in hubverse lingo as the `task-ID` variables.
