...
The function will validate a given config file against a specific version of it's schema, specified through argument `schema_version`. The default value of `schema_version` is `"from_file"` which uses the version specified in the `schema_version` property of the config file being validated. 
...
You can suppress this by using `pretty_errors = FALSE`.
