test_that("merging-splitting model_id works", {
    ...
    # Test splitting
    expect_snapshot(split_model_id(tbl) %>%
                        dplyr::arrange(rev(model_abbr)))
    tbl <- split_model_id(tbl)
    expect_equal(
        names(tbl),
        c(
    ...
    expect_snapshot(split_model_id(tbl), error = TRUE)

    # Test merging
    expect_equal(
        names(suppressMessages(merge_model_id(tbl))),
        c(
    ...
    expect_equal(
        suppressMessages(unique(merge_model_id(tbl)$model_id)),
    ...
    expect_snapshot(merge_model_id(tbl), error = TRUE)
